...
def save_in_excel(filename, database):
    if filename.split('.')[-1]!='xls' and filename.split('.')[-1]!='xlsx':
        print ("Filename does not have correct extension.")
        return None
...
def search_movie(title):
    ...
    try:
        ...
        print(f'Tretrieving the data of "{title}" now...')
        ...
        if json_data['Response'] == 'True':
            ...
            # pta czy pobrac plakat
            if json_data['Poster']!='N/A':
                poster_yes_no = input ("Chcesz plakat? y or no: ").lower()

                if poster_yes_no == 'y':
                    save_poster(json_data)
            #zapisac w bazie?
            save_database_yes_no = input('Save the info? y or no: ' ).lower()
            if save_database_yes_no == 'y':
...
print_database('filmy.sqlite')
save_in_excel('test.xlsx','filmy.sqlite')
import pandas as pd
